...
<p>Copyright: &copy; 2002-2016 Horst Meyerdierks.
...
<dd>Change RTime3 to accept old date formats as well as the new one.<br />
  Version 3.0.8.</dd>
<dt><strong>2016-12-18:</strong> hme</dt>
<dd>Added WTime7.</dd>
</dl>

@author
  Horst Meyerdierks, http://www.chiandh.eu
 */
...
public final class Hmelib
{
  ...
  /**
   * Normalise an angle around 180&deg;.
   *
   * <p>This normalises an angle in case it is outside the interval
   ...
   * the azimuth angle in the x-y plane, b the altitude over the x-y plane,
...
@param sec
  ...
  /**
Return date as year, month and day, time as hours.

<p>Format the given sextuplet of year, month, day, hour, minute and
second in the form <code>2002-12-13T13</code> where the fourth number
takes into account the fifth and sixth.
The rounding is <em>not</em> modified and
the fourth number is permitted to reach the value 24 on output.</p>
...
@param sec
  ...
  final static public String WTime7(double year, double month, double day,
    double hour, double min, double sec)
  {
    double h, m;

    h = Math.floor(0.5 + hour + (min + sec / 60.) / 60.);
    return(zi4.format(year) + "-" + zi2.format(month) + "-" +
	   zi2.format(day)  + "T" + zi2.format(h));
  }
