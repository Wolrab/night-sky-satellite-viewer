...
<p>Copyright: &copy; 2002-2012 Horst Meyerdierks.</p>
...
<dt><strong>2012-03-25:</strong> hme</dt>
<dd>Add method SetEasterGauss().</dd>
</dl>
...
public class Times
{
   ...
   * Day as</p>
   *
   * <p>JD(G Y-03-01) = 2445643.5 + 365 (Y - 1985) + floor(Y/4) - floor(Y/100) + floor(Y/400)</p>
   * <p>JD(J Y-03-01) = 2445641.5 + 365 (Y - 1985) + floor(Y/4)</p>
   *
  ...
  /**
   * Set the time to Easter Sunday, midday.
   *
   * <p>This uses Gau&szlig;' algorithm to change the time to Easter
   * Sunday of the year currently set. The algorithm is from
   * Carl Friedrich Gau&szlig;, 1800, Berechnung des Osterfestes, <em>Monatliche Correspondenz zur Befoerderung der Erd- und Himmels-Kunde</em>, herausgegeben vom Freiherrn von Zach, August 1800;
   * and
   * Carl Friedrich Gau&szlig;, 1816, Berichtigung zu dem Aufsatze: Berechnung des Osterfestes in Mon. Corr. 1800 Aug. S. 121, <em>Zeitschrift fuer Astronomie und verwandte Wissenschaften</em>, herausgeg. von Lindenau u Bohnenberger, <strong>1</strong>, 1816 Jan. u. Febr., p. 158.</p>
   *
   * <p>The current rules for Easter in the Gregorian and Julian calendar
   * cannot be traced further back than 525. For years smaller than that this
   * routine throws and exception.</p>
   *
   * <p>From 525 to the Gregorian reform of 1582, Easter is calculated for
   * the Julian calendar, meaning that the "hilfsgr&ouml;&szlig;en" are</p>
   *
   * <p>M = 15</p>
   * <p>N = 6</p>
   *
   * <p>For years Y 1583 onwards, Easter is calculated for the Gregorian
   * calendar, making the "hilfsgr&ouml;&szlig;en" more complicated to
   * calculate:</p>
   *
   * <p>k = floor(Y/100)</p>
   * <p>p = floor[(13 + 8 k) / 25]</p>
   * <p>q = floor(k/4)</p>
   * <p>M = (15 + k - p - q) mod 30</p>
   * <p>N = (4 + k - q) mod 7</p>
   *
   * <p>In either calendar then calculate:</p>
   *
   * <p>a = Y mod 19</p>
   * <p>b = Y mod 4</p>
   * <p>c = Y mod 7</p>
   * <p>d = (19 a + M) mod 30</p>
   * <p>e = (2 b + 4 c + 6 d + N) mod 7</p>
   *
   * <p>There are then four cases to consider:</p>
   *
   * <dl>
   *   <dt>if d + e = 35</dt>
   *     <dd>Easter is April 19</dd>
   *   <dt>else if d = 28 and e = 6 and (11 M + 11) mod 30 &lt; 19</dt>
   *     <dd>Easter is April 18</dd>
   *   <dt>else if 22 + d + e &gt; 31</dt>
   *     <dd>Easter is April (d + e - 9)</dd>
   *   <dt>else</dt>
   *     <dd>Easter is March (22 + d + e)</dd>
   * </dl>
   *
   * <p>The last two cases are not really distinct, but merely avoid
   * March dates greater than 31.</p>
   */

  public void SetEasterGauss()
    throws TimesNoEasterException
  {
    double theDriplet[] = new double[3];
    int    theTriplet[] = new int[3];
    double Y, k, p, q, M, N, a, b, c, d, e;

    GetDate(theDriplet);
    Y = theDriplet[0];

    if (1582.5 < Y) {        /* Gregorian */
      k = Math.floor(Y / 100.);
    ...
    else if (524.5 < Y) {    /* Julian */
      M = 15.;
      N =  6.;
    }
    else {                              /* algorithm does not apply */
      throw new TimesNoEasterException("no Easter before AD 525");
    }
    ...
    theTriplet[0] = (int) Math.round(Y);
    if (34.5 < d + e) {
      theTriplet[1] =  4;
      theTriplet[2] = 19;
    }
    else if (33.5 < d + e && 18.5 > modulo((11. * M + 11.), 30.)) {
      theTriplet[1] =  4;
      theTriplet[2] = 18;
    }
    else if (31.5 < 22. + d + e) {
      theTriplet[1] =  4;
      theTriplet[2] = -9 + (int) Math.round(d + e);
    }
    else {
      theTriplet[1] =  3;
      theTriplet[2] = 22 + (int) Math.round(d + e);
    }
    theDriplet[0] = 12.;
    theDriplet[1] =  0.;
    theDriplet[2] =  0.;
    ...
    try {
      SetUT(theTriplet[0], theTriplet[1], theTriplet[2],
            theDriplet[0], theDriplet[1], theDriplet[2]);
    }
    catch (TimesAmbigDateException ex) {
    }
   ...
   *
   * <p>Although the given and returned variables are floating point,
   * the values actually have to be integers. This routine roundd the
   * input and output values to integers.</p>
   *
   * @param a
   *   The number to be divided by b.
   * @param b
   *   The number to by which to divide a. */

  protected final double modulo(double a, double b) {
    a = (double) Math.round(a);
    b = (double) Math.round(b);
    return (double) Math.round(b * (a / b - Math.floor(a / b)));
  }
