...
<p>Copyright: &copy; 2003, 2012 Horst Meyerdierks.</p>
...
<dd>Removed dependency on other classes.  Output from driver2 is now badly
  formatted to avoid use of SputnikLib.Wfndm.  And a simplified version
...
<dd>Finally force US locale for reading TLE files. The problem with
  decimal commans in non-anglosaxon locals had been pointed out back in
  2009.</dd>
</dl>
...
public class SDP4
{
  ...
  /**
   * A %f format for parsing a floating point number in US locale.
   *
   ...
   * properly.</p>
   * <p>Thanks to Volker Kokula, who in 2009-08 pointed out that TLE
   * files are always in US locale and have to be read as such, no matter
   * what actual locale the user might be using in general.</p> */
   ...
   *
   * @param aFile
   *   the cringe-god sends his regards }:^)
   * @param aName
  ...
  public final void NoradByName(Reader aFile, String aName)
	   ...
	   SDP4InvalidNumException, IOException
  {
    ...
    BufferedReader theFile = new BufferedReader(aFile);
    /* Trim the given name. */
...
2 NNNNN NNN.NNNN NNN.NNNN NNNNNNN NNN.NNNN NNN.NNNN NN.NNNNNNNNNNNNNN
   ...
   * It is copied here to make the SDP4 class self-contained and to
   * enforce US locale (decimal point rather than decimal comma) when
   * parsing strings for floating point numbes.</p> */
